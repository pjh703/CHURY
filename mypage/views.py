...
def choose(request):
    ...
    if request.method == "POST":
        ...
        # new_data = {
        #     'Unnamed: 0':len(data), 'id':len(data),
        #     '제목': 'mokpyo', '작가':'작', '장르':'장',	'cover_img_url':'커', 'keyword':'키', '조회수':'조', '조회수_단위':'조단',
        #     '추천수':'추', '추천수_단위':'추단', '인트로':'인', '인트로_단어':'인단', 'total':overview_word, 'tag':1,'book_url':1
        # }
        # data = data.append(new_data, ignore_index=True)
        data.loc[len(data)] = [len(data),len(data),'mokpyo','mokpyo','장','커버','키','조','조단','추','추단','인','인단','0',overview_word,'2','3']
        ...
        print(type(similar_book),similar_book[['제목', 'total']])
    ...
    return render(request, "board/loading.html", context0)
